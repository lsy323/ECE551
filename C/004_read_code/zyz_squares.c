...
bool isAnd(bool b1, bool b2){
	return b1 && b2;
}
bool isOr(bool b1, bool b2) {
	return b1 || b2;
}
...
void squares(int size1, int x_offset, int y_offset, int size2) {
  ...
  for(int y = 0;y < h;y++){  //count from 0 to h. Call the number you count with y
    ...
    for(int x = 0;x < w;x++){    //count from 0 to w. Call the number you count with x
      ...
      if(isAnd(isAnd(x >= x_offset,x < x_offset+size2), isOr(y==y_offset,y==y_offset+size2-1))
      ||isAnd(isAnd(y>=y_offset, y<y_offset+size2), isOr(x==x_offset,x==x_offset+size2-1))){
      	printf("*");
      }else if(isAnd(x<size1,y==0||y==size1-1) || isAnd(y<size1,x==0||x==size1-1)){
      	printf("#");
      }else{
      	printf(" ");
      }
